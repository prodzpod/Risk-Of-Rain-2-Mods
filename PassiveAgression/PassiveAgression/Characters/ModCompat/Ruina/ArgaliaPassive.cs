...
namespace PassiveAgression.ModCompat
{
    public static class ArgaliaPassive
    {
        ...
        static void UnProwess(ILContext il)
        {
            ...
            if (c.TryGotoNext(MoveType.After, x => x.MatchCallOrCallvirt(typeof(Component).GetMethod("GetComponent", new Type[] { }).MakeGenericMethod(typeof(RedMistStatTracker)))))
            { // && c.TryGotoNext(MoveType.AfterLabel,x => x.MatchRet())){
